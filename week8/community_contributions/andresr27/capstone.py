...
def reformat(message):
    """Basic log message reformatting for HTML display."""
    if "[INFO]" in message:
        return f'<span style="color: #4CAF50;">{message}</span>'
    if "[ERROR]" in message:
        return f'<span style="color: #f44336;">{message}</span>'
    if "[WARNING]" in message:
        return f'<span style="color: #ff9800;">{message}</span>'
    return message
        ...
        with gr.Blocks(title="The Price is Right", fill_width=True) as ui:
            ...
            def get_plot():
                try:
                    ...
                    logging.warning(f"Could not get plot data: {e}")
                    return get_initial_plot()
            ...
            def do_select(selected_index: gr.SelectData):
                ...
                if row < len(opportunities):
                    ...
                    if framework.planner and framework.planner.messenger:
                        ...
                        logging.warning("Planner or Messenger not initialized.")
