...
def process_with_rag(text_input, audio_input, chat_history, platform, use_rag):
    ...
    try:
        ...
        messages = [
            {
                "role": "system",
                "content": "You are a technical teaching assistant for the 'Become an LLM Engineer' course."
            }
        ]
...
# Build UI
with gr.Blocks(title="AI Tutor Chat", theme=gr.themes.Soft(), css="button.primary { background-color: lightblue !important; } button.secondary { background-color: lightblue !important; }") as ui:
    gr.Markdown("# AI Tutor Chat")
    gr.Markdown("Ask questions about code, LLMs, or any technical topic.")
    ...
    with gr.Row():
        ...
        with gr.Column(scale=1, min_width=300):
            ...
            # RAG Controls
            with gr.Group():
                gr.Markdown("### Course Knowledge Base (RAG)")
                ...
                with gr.Row():
                    use_rag = gr.Checkbox(
                        label="Enable Course RAG",
                        value=True,
                        info="Use course curriculum and technical docs to answer questions"
                    )
